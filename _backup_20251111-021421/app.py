...
# --- local modules ---
from config import Config            # ถ้า config แยกไฟล์; ถ้าอยู่ไฟล์เดียวให้คงไว้ตามที่คุณเขียน
from extensions import db, migrate
from models import Drug, Compatibility, AccessLog

# --- สร้างแอปรูปแบบ global เพื่อให้ decorators @app.route ใช้งานได้ ---
app = Flask(__name__)
app.config.from_object(Config)
db.init_app(app)
migrate.init_app(app, db)

# --- ถ้าคุณตั้งใจใช้ typing ในฟังก์ชันด้านล่าง เช่น -> Path จะไม่ error แล้ว ---
...
# ==== imports ที่ต้องมีเพื่อกัน error ตอน import แอป ====
import numpy as np  # กัน NameError: np not defined จากที่เคยเจอ
# ถ้ามี click.argument ที่เคยโดน NameError: ใช้จาก click โดยตรง
from click import argument  # ถ้าคุณมี @argument(...) อยู่ในไฟล์

# ==== ถ้าที่บนไฟล์มี app = Flask(__name__) อยู่แล้ว ข้ามได้ ====
# from flask import Flask
# app = Flask(__name__)

# ==== SQLAlchemy / Migrate binding (ถ้ามีอยู่แล้ว ข้ามส่วนนี้ได้) ====
try:
    from flask_sqlalchemy import SQLAlchemy
    from flask_migrate import Migrate
    # ถ้าโปรเจกต์คุณผูก db/migrate ที่อื่นแล้ว ให้ข้าม 4 บรรทัดนี้
    ...
    # ถ้ายังไม่ได้ติดตั้งแพ็กเกจ จะไม่ล้มตอน import แอป
    pass

# ==== เพิ่ม factory แบบบางสุด ====
def create_app():
    # ถ้าใช้ blueprint/register อะไร ให้แน่ใจว่าโค้ดนั้นรันก่อนถึงบรรทัดนี้
    return app
...
def _id_map_path() -> Path:
    # เก็บ mapping id seed → id จริง หลัง seed-drugs
    return _data_dir() / "id_map.json"
...
def status_to_code(s: str) -> str:
    ...
    # Incompatible
    if t.startswith("incomp") or t in {"i", "no", "false", "0", "ห้ามผสม", "ไม่เข้ากัน"}:
        return "I"
    ...
    # No data / unknown
    if t in {"nd", "unknown", "no data", "ไม่มีข้อมูล", "na", "n/a", "-"}:
        return "ND"

    # ค่าอื่นๆที่ไม่รู้จัก → ND (หรือจะเปลี่ยนเป็น raise ValueError ก็ได้)
    return "ND"
...
def medication_administration():
    meds = [
        ...
        # C
        {"label": "Cefazolin", "endpoint": "cefazolin_route"},  # ← เดี๋ยวเพิ่ม route ด้านล่าง
        ...
        # R
        {"label": "Remdesivir", "endpoint": "remdesivir_route"},  # ← แก้สะกด
        # S
        ...
        # U
        {"label": "Unasyn", "endpoint": "unasyn_route"},         # ← แก้สะกด
...
def log_request():
    if request.endpoint != "static":
        try:
            ...
            # อย่าทำให้ request หลักพังเพราะ logging
            db.session.rollback()
...
@app.route('/cefazolin', methods=['GET', 'POST'])
def cefazolin_route():
    dose = None
    result_ml = None
    error = None
    # ตัวอย่าง: stock 1000 mg / 10 mL → 100 mg/mL
    try:
        if request.method == 'POST':
...
# --- helper: ดึง label ของยาแบบกันตาย ---
def _drug_label_by_id(drug_id: int) -> str:
    if not drug_id:
        return "—"
    obj = Drug.query.get(drug_id)
    if not obj:
        return f"ID {drug_id}"

    # ชื่อคีย์ที่พบบ่อย ใส่ตามที่สคีมาคุณใช้ได้เลย
    for field in (
        "name", "drug_name", "generic_name", "display_name",
        "thai_name", "th_name", "en_name", "label", "title"
    ):
        val = getattr(obj, field, None)
        if isinstance(val, str) and val.strip():
            return val.strip()

    # บางสคีมาใช้คอลัมน์ชื่อ 'drug' เป็นข้อความ
    val = getattr(obj, "drug", None)
    if isinstance(val, str) and val.strip():
        return val.strip()

    # เผื่อโมเดลทำ __str__ ไว้
    try:
        s = str(obj).strip()
    ...
    return f"Drug {getattr(obj, 'id', drug_id)}"
...
def compatibility_check():
    ...
    # หาเรคอร์ดด้วยคอลัมน์ id (อย่าใช้ relationship ตรง ๆ)
    row = (db.session.query(Compatibility)
           .filter(
               or_(
                   and_(Compatibility.drug_id == drug_id,
                        Compatibility.co_drug_id == co_id),
                   and_(Compatibility.drug_id == co_id,
                        Compatibility.co_drug_id == drug_id)
               )
           )
           .first())

    # ชื่อยา—ใช้ helper ที่ทนต่อความต่างของสคีมา
    d1 = _drug_label_by_id(drug_id)
    d2 = _drug_label_by_id(co_id)

    # map สถานะ → code สำหรับสี
    raw_status = (row.status if row and row.status else
                  row.status_code if row and getattr(row, "status_code", None) else
                  "ND")
    raw_status = str(raw_status).strip().upper()
    status_map = {
        "YES": "C", "NO": "I", "UNCERTAIN": "U", "UNKNOWN": "ND",
        "COMPATIBLE": "C", "INCOMPATIBLE": "I",
        "C": "C", "I": "I", "U": "U", "ND": "ND"
    }
    code = status_map.get(raw_status, "ND")

    meta = row.meta if (row and getattr(row, "meta", None)) else {}
    note = row.note if (row and getattr(row, "note", None)) else ""

    return render_template(
        "compatibility_result.html",
        code=code,                # ใช้กำหนดสี .C .I .U .ND ในเทมเพลต
        status=raw_status,
        drug1_name=d1,            # ชื่อยาฝั่งซ้าย
        drug2_name=d2,            # ชื่อยาฝั่งขวา
        meta=meta,
        note=note,
    )
...
def seed_compat_from_names():
    ...
    db.session.commit()
    total = Compatibility.query.count()
    print(f"✅ seed(by names): inserted={ins} updated={upd} skipped={skip} total={total}")
...
@click.option("--path", default="data/compatibility.csv", show_default=True,
...
@click.option("--default-unknown/--no-default-unknown", default=True, show_default=True,
        help="ถ้า status ว่าง ให้ลงเป็น Unknown")
@click.option("--commit-interval", default=2000, show_default=True,
...
@click.option("--out", default="data/compatibility.csv", show_default=True,
        help="ไฟล์ CSV ปลายทาง (id,drug,co_drug,status,note)")
@click.option("--status-format", type=click.Choice(["standard","legacy"]), default="legacy", show_default=True,
        help="รูปแบบค่า status: standard=Compatible/… หรือ legacy=Yes/No/…")
@click.option("--only-unknown/--all", default=False, show_default=True,
