...
# ====== Compatibility utilities & meta loader (PUT THIS NEAR THE TOP) ======
...
def canonicalize_name(s: str) -> str:
    ...
    low = low.replace("meropenam", "meropenem")  # กันสะกดผิดยอดฮิต
...
def status_to_code(s: str) -> str:
    """แปลงรูปแบบสถานะที่หลากหลาย → โค้ด 4 ค่า: C/I/U/ND (ทนต่อ Yes/No, C/I, ไทย)"""
    if not s:
        return "ND"
    t = str(s).strip().lower()
    ...
    if t in {"nd", "unknown", "no data", "ไม่มีข้อมูล"}:
        return "ND"
    # เผื่อค่าอื่น ๆ ที่หลุดมา → Unknown
    return "ND"

# ---------- อ่าน meta อ้างอิงจาก data/seed_compatibility.json (ทำคีย์สมมาตร) ----------
...
def medication_administration():
    ...
    groups = group_meds_by_letter(meds)   # <-- ตอนนี้รองรับ label แล้ว
    letters = list(groups.keys())         # เรียงตาม A..Z และ '#'
...
# init extensions
# DUP: # DUP: # DUP: db.init_app(app)
# DUP: # DUP: # DUP: migrate.init_app(app, db)
